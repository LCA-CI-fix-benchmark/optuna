...
from packaging import ve``lightning.pytorch.LightningModule.validation_epoch_end`` and the names thus depend on
how this dictionary is formatted.

.. note::
    For distributed data parallel training, the PyTorch Lightning version needs to be
    higher than or equal to v1.6.0. Also, :class:`~optuna.study.Study` should be
    instantiated with RDB storage.

.. note::
    If you want to use PyTorchLightningPruningCallback in a distributed training
    environment, you need to invoke `PyTorchLightningPruningCallback.check_pruned()`
    manually to properly handle :class:`~optuna.exceptions.TrialPruned`.
...
class PyTorchLightningPruningCallback:
    ...
    def on_fit_start(self, trainer: Trainer, pl_module: "pl.LightningModule") -> None:
        ...
        if self.is_ddp_backend:
            if version.parse(pl.__version__) < version.parse(
                "1.6.0"
            ):
    ...
    def check_pruned(self) -> None:
        ...
        ``lightning.pytorch.Trainer.fit()``.ge import RDBStorage
